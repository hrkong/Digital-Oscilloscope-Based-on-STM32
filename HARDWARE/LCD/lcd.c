...
void LCD_DrawLine_CLR(u16 x1, u16 y1, u16 x2, u16 y2)
{
	u16 t; 
	int xerr=0,yerr=0,delta_x,delta_y,distance; 
	int incx,incy,uRow,uCol; 
...
void LCD_ShowChar_COLOR(u16 x,u16 y,u8 num,u8 size,u8 mode,u16 color)
{  							  
    u8 temp,t1,t;
	u16 y0=y;
	u8 csize=(size/8+((size%8)?1:0))*(size/2);		//得到字体一个字符对应点阵集所占的字节数	
...
void LCD_ShowString_COLOR(u16 x,u16 y,u16 width,u16 height,u8 size,u8 *p,u16 color)
{         
	u8 x0=x;
	width+=x;
	height+=y;
    ...
    {       
        if(x>=width){x=x0;y+=size;}
        if(y>=height)break;//退出
        LCD_ShowChar_COLOR(x,y,*p,size,0,color);
        x+=size/2;
        p++;
    }  
}
